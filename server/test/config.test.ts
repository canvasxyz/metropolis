...
describe("Config", () => {
  ...
  describe("getServerNameWithProtocol", () => {
    ...
    test("returns given req domain when DEV_MODE is true", async () => {
      ...
      expect(Config.getServerNameWithProtocol(req)).toBe(
        "https://mydomain.xyz"
      );
    });
  ...
  describe("getServerUrl", () => {
    ...
    test("returns API_DEV_HOSTNAME when DEV_MODE is true", async () => {
      jest.replaceProperty(process, "env", {
        DEV_MODE: "true",
        API_DEV_HOSTNAME: "dev.example.com",
      });
    ...
    test("returns http://localhost:8040 when DEV_MODE is true and DEV_URL is not set", async () => {
      jest.replaceProperty(process, "env", { DEV_MODE: "true" });

      const { default: Config } = await import("../src/config");

      expect(Config.getServerUrl()).toBe("http://localhost:8040");
    });
  });
