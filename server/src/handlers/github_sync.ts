    ...
    for (const pull of pulls) {
      ...
      if (existingConversation) {
        if (!existingConversation.github_sync_enabled) {
          console.log(
            `github sync is disabled for PR ${pull.number} ${pull.head?.label}, skipping`,
          );
          continue;
        }
        ...
        if (pull.state == "open") {
          console.log(
            `conversation with PR id ${pull.number} ${pull.head?.label} is open, updating`,
          );
          // get fip
          let fipFields;
          try {
          ...
          console.log(
            `conversation with PR id ${pull.number} ${pull.head?.label} is closed, updating`,
          );
