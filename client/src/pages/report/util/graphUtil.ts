...
const graphUtil = (comments, math, badTids) => {
  ...
  var maxCommentX = _.maxBy(commentsPoints, (pt) => {
    return pt.x
  }).x
  var minCommentX = _.minBy(commentsPoints, (pt) => {
    return pt.x
  }).x
  var maxCommentY = _.maxBy(commentsPoints, (pt) => {
    return pt.y
  }).y
  var minCommentY = _.minBy(commentsPoints, (pt) => {
    return pt.y
  }).y
