...
const graphUtil = (comments, math, badTids) => {
  ...
  for (let i = 0; i < projX.length; i++) {
    if (comments[i]) {
      const tid = comments[i].tid
      const index = tidToIndex[tid]
      const x = projX[index]
      const y = projY[index]
  ...
  const greatestAbsPtptX = Math.abs(
    _.maxBy(baseClusters, (pt) => {
      return Math.abs(pt.x)
    }).x
  )
  const greatestAbsPtptY = Math.abs(
    _.maxBy(baseClusters, (pt) => {
  ...
  const xx = scaleLinear()
    .domain([-greatestAbsPtptX, greatestAbsPtptX])
    .range([border, globals.side - border])
  const yy = scaleLinear()
    .domain([-greatestAbsPtptY, greatestAbsPtptY])
    .range([border, globals.side - border])

  const xCenter = globals.side / 2
  const yCenter = globals.side / 2

  const maxCommentX = _.maxBy(commentsPoints, (pt) => {
    return pt.x
  }).x
  const minCommentX = _.minBy(commentsPoints, (pt) => {
    return pt.x
  }).x
  const maxCommentY = _.maxBy(commentsPoints, (pt) => {
    return pt.y
  }).y
  const minCommentY = _.minBy(commentsPoints, (pt) => {
    return pt.y
  }).y
  ...
  // globals.side - border - xCenter = xScale * maxCommentX
  const xScaleCandidateForRightSide = (globals.side - border - xCenter) / maxCommentX
  const yScaleCandidateForBottomSide = (globals.side - border - yCenter) / maxCommentY

  // xLowestMapped = xCenter + xScale * minCommentX
  // border = xCenter + xScale * minCommentX
  // border - xCenter = xScale * minCommentX
  // (border - xCenter) / minCommentX = xScale
  const xScaleCandidateForLeftSide = (border - xCenter) / minCommentX
  const yScaleCandidateForTopSide = (border - yCenter) / minCommentY

  const commentScaleupFactorX = Math.min(
    Math.abs(xScaleCandidateForRightSide),
    Math.abs(xScaleCandidateForLeftSide)
  )

  const commentScaleupFactorY = Math.min(
