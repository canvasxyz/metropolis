...
class VoronoiCells extends React.Component<{
  ...
  getFill(cell) {
    ...
    if (
      this.props.currentBeeswarmComment &&
      this.props.currentBeeswarmComment.tid === cell.data.tid
    ) {
      return "rgb(255,0,0)"
    } else {
      return "black"
        ...
        {this.props.voronoi.map((cell, i) => {
          return (
            <g key={i} onMouseEnter={this.props.onHoverCallback(cell)}>
              ...
              <circle r={3} fill={this.getFill(cell)} cx={cell.data.x} cy={cell.data.y} />
            </g>
          )
        })}
      </g>
...
> {
  ...
  svgHeight: number
  margin: any
  widthMinusMargins: number
  heightMinusMargins: number
    ...
    return (
      <div style={{ width: this.svgWidth }}>
        ...
        <p style={globals.paragraph}>
          ...
          to find out what the most divisive statement was.
        </p>
              ...
              <VoronoiCells
                ...
                probabilities={
                  this.state.currentBeeswarmComment
                    ? this.props.probabilities[this.state.currentBeeswarmComment.tid]
                    : null
                }
                currentBeeswarmComment={this.state.currentBeeswarmComment}
                voronoi={this.state.voronoi}
                onHoverCallback={this.onHoverCallback.bind(this)}
              />
            ) : null}
          </g>
          <line
            x1="0"
            y1={this.svgHeight - 10}
            x2={this.svgWidth}
            y2={this.svgHeight - 10}
            strokeWidth="1"
            stroke="black"
          />
        </svg>
